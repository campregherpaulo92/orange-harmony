...
st.markdown('<div class="oh-section-title"><span class="oh-title-icon">🎤</span><span class="oh-title-text">Coach Virtual e Analista de Evolução Musical</span><span class="oh-title-line"></span></div>', unsafe_allow_html=True)
...
def _player_espectro(src_audio, cor, uid):
    # Player com espectro animado em tempo real (Web Audio API) via base64
    try:
...
7. **🔄 Conversor**: Converte áudio entre WAV e MP3.
8. **🎛️ Produção**: Estúdio que gera backing track (baixo, bateria e acordes) no tom e BPM detectados da gravação, em vários estilos (Pop, Rock, Balada, Sertanejo, Funk, MPB, Gospel, Reggae, Blues, Jazz, Forró, Eletrônica).
9. **🎛️ Orange Studio (botão flutuante)**: separação de voz/instrumental (stems) via Demucs rodando no Colab. Abre integrado na própria página (sem janela modal). O usuário cola a URL gradio.live, grava ou sobe o áudio e separa; as faixas de Vocals e Instrumental aparecem com player de espectro animado, mesmo depois de fechar o Estúdio.
...
with tab_producao:
    ...
    if st.button("🎛️ Gerar produção", type="primary"):
        ...
        st.info("💡 A separação de stems (voz/violão separados) agora fica no botão flutuante 🎛️ Estúdio.")
        # ══════════════════ ASSISTENTE VIRTUAL (laranjinha com memória e chats) ══════════════════
def _encontrar_laranjinha():
    nomes = ["laranjinha.png", "laranjinha.PNG", "Laranjinha.png",
             "laranjinha_transparente.png", "laranjinha_transparente.PNG",
...
# ── CSS do balão do diálogo da Laranjinha (via st.markdown, para valer sem iframe) ──
st.markdown("""
<style>
[data-testid="stDialog"] {
    position: fixed !important;
...
with tab_ia_compositora:
    ...
    if st.button("🤖 Gerar música com IA", type="primary"):
        ...
        st.info("💡 Dica: quanto mais específico o prompt (estilo, instrumentos, clima, BPM), melhor o resultado.")
# ══════════════════════════════════════════════════════════════
# ESTÚDIO DE STEMS — integrado na página (sem modal), botão flutuante
# ══════════════════════════════════════════════════════════════
import base64 as _b64_mod
try:
    import requests as _requests
...
def _player_espectro(src_audio, cor, uid):
    ...
    try:
        ...
        return False
    # Limite ampliado: stems de ~1,2 MB (~1,6 MB em base64) passam sem problema
...
def _renderizar_secao_estudio():
    """Renderiza o Orange Studio inteiro, integrado na página (sem st.dialog)."""
    st.markdown(_CSS_FAIXAS, unsafe_allow_html=True)

    col_header, col_fechar = st.columns([6, 1])
    with col_header:
        st.markdown("""
        <div class="oh-studio-top">
            <span class="st-title">🎛️ Orange Studio</span>
        ...
        """, unsafe_allow_html=True)
    with col_fechar:
        if st.button("✕ Fechar Estúdio", key="fechar_estudio_btn"):
...
# ── Renderização condicional: Estúdio aberto OU só as faixas no rodapé ──
if st.session_state.get("estudio_aberto"):
    _renderizar_secao_estudio()
else:
    _renderizar_faixas_estudio()

# ── Botão flutuante do Estúdio (mesmo padrão da Laranjinha, sem dialog) ──
st.markdown('<div id="fab-estudio"></div>', unsafe_allow_html=True)
if st.button("", key="abrir_estudio", help="Abrir Estúdio"):
